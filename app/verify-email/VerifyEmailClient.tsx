  ...
  if (status === 'loading') {
    return (
      <div className="flex flex-col items-center gap-4 text-gray-600 dark:text-gray-400">
        <Loader2 className="w-12 h-12 animate-spin text-emerald-600" aria-hidden="true" />
  ...
  if (status === 'no-token') {
    return (
      <div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-800 p-8 text-center space-y-4">
        <Mail className="w-16 h-16 text-emerald-600 dark:text-emerald-400 mx-auto" aria-hidden="true" />
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Check your email</h1>
        <p className="text-gray-600 dark:text-gray-400 text-sm">
          We sent a verification link to your email address. Click the link to activate your account.
        </p>
        <Link href="/login" className="inline-block mt-4 text-emerald-600 dark:text-emerald-400 font-medium hover:underline text-sm">
          Back to sign in
        </Link>
      </div>
    )
